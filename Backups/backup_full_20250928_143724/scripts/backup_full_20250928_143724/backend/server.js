  ...
  try {
    ...
    if (deletedCount > 0) {
      ...
      try {
        ...
        fs.appendFileSync(
          path.join(LOG_DIR, 'documents.log'),
          `[${new Date().toISOString()}] User ${userId} deleted: ${fileName}n`
        );
      } catch (error) {
